...
10.06.2023 : added Arduino Mega/Nano/Uno version which requires TTL logic converter for 3.3V - TXS0108E. Added ESP32 version. Changed RECRAW command to start recording RAW signal once something appears over the radio. Added command ADDRAW to manual compose the signal in the buffer (by copying hex number chunks from Universal Radio Hacker tool for example). Added option SCAN to find a peak frequency for recording/jamming..
