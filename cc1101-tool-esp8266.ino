...
static void exec(char *cmdline) {
  ...
  if (strcmp_P(command, PSTR("help")) == 0) {
    ...
    for (int i = 0; i < RECORDINGBUFFERSIZE; i = i + 32) {  // first convert to hex numbers
    ...
    if ((len <= 120) && (len > 0)) {
      ...
      if ((bigrecordingbufferpos + len) < RECORDINGBUFFERSIZE) {  // copy current frame and increase pointer for next frames
    ...
    if (recordingmode == 1) {
      ...
      for (int i = 0; i < RECORDINGBUFFERSIZE; i++) { bigrecordingbuffer[RECORDINGBUFFERSIZE] = 0; };
      recordingmode = 1;
      jammingmode = 0;
      receivingmode = 0;
    ...
    if (framesinbigrecordingbuffer > 0) {
      ...
      for (setting = 1; setting <= framesinbigrecordingbuffer; setting++) {
        ...
        if ((len <= 60) and (len > 0)) {
          // take next frame from the buffer  for replay
          // flush textbuffer
          for (setting2 = 0; setting2 < BUF_LENGTH; setting2++) { textbuffer[setting2] = 0; };
...
void setup() {
  ...
  telnetServer.begin();
  Serial.print("Telnet server started on port 23\n");
      ...
      while (telnetClient.connected()) {
        ...
        while (telnetClient.available()) {
          ...
          if (chatmode == 1) {
            ...
            // clear telnetClient port buffer index
            i = 0;

            // something was received over telnetClient port put it into radio sending buffer
            while (telnetClient.available() and (i < (CCBUFFERSIZE - 1))) {
              // read single character from telnetClient port
              ccsendingbuffer[i] = telnetClient.read();

              // also put it as ECHO back to telnetClient port
              telnetClient.write(ccsendingbuffer[i]);

              // if CR was received add also LF character and display it on telnetClient port
              if (ccsendingbuffer[i] == 0x0d) {
                telnetClient.write(0x0a);
                i++;
          ...
          // handling CLI commands processing
            ...
            int data = telnetClient.read();
            if (data == '\b' || data == '\177') {  // BS and DEL
              if (length) {
                length--;
                if (do_echo) telnetClient.write("\b \b");
              }
            } else if (data == '\r' || data == '\n') {
              if (do_echo) telnetClient.write("\r\n");  // output CRLF
              buffer[length] = '\0';
              if (length) exec(buffer);
              length = 0;
            } else if (length < BUF_LENGTH - 1) {
              buffer[length++] = data;
              if (do_echo) telnetClient.write(data);
            }
          };
        ...
        if (ELECHOUSE_cc1101.CheckReceiveFlag() && (receivingmode == 1 || recordingmode == 1 || chatmode == 1)) {
          ...
          if (ELECHOUSE_cc1101.CheckCRC()) {
            ...
            if ((chatmode == 1) && (len < CCBUFFERSIZE)) {
              ...
              telnetClient.print((char *)ccreceivingbuffer);
            ...
            if (((receivingmode == 1) && (recordingmode == 0)) && (len < CCBUFFERSIZE)) {
              ...
              ccreceivingbuffer[len] = '\0';
              // flush textbuffer
              for (int i = 0; i < BUF_LENGTH; i++) { textbuffer[i] = 0; };
              ...
              telnetClient.print((char *)textbuffer);
              // set RX  mode again
              ELECHOUSE_cc1101.SetRx();
            ...
            if (((recordingmode == 1) && (receivingmode == 0)) && (len < CCBUFFERSIZE)) {
              ...
              if ((bigrecordingbufferpos + len + 1) < RECORDINGBUFFERSIZE) {  // put info about number of bytes
                ...
                telnetClient.print(F("Recording buffer full! Stopping..\r\nFrames stored: "));
                telnetClient.print(framesinbigrecordingbuffer);
